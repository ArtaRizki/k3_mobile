...
class InspectionProjectCreateController extends GetxController {
  ...
  final projectNameC = TextEditingController().obs,
  ...
  init() async {
    ...
    if (Get.arguments != null) {
      ...
      projectNameC.value.text = data?.proyek ?? '';
  ...
  Future<void> getKategori() async {
    if (!loading.value) {
      ...
      final response = await httpClient.get('/get-data-kategori-inspeksi');
      if (response.statusCode == 200) {
        categoryList.value =
  ...
  bool validate() {
    if (projectNameC.value.text.isEmpty) return false;
  ...
  Future<void> sendInspectionProject() async {
    ...
    final httpClient = HttpRequestClient();
    final response = await httpClient.post('/create-inspeksi', body: body);
    ...
    if (response.statusCode == 201 || response.statusCode == 200) {
      ...
      update();
      loading(false);
      Get.find<InspectionProjectController>().getData();
      Get.back();
